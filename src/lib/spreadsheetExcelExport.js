...
const COLUMNS = [
  ...
  ['A4 Round-Trip (kgCO2e, reference)', 'a4', 'transport'],
  ['A4 Consolidated (kgCO2e, reported)', 'a4Consolidated', 'transport'],
...
export async function exportReferenceMatchingExcel(rows, meta, filename = 'LCA-Table-Project-Analysis_v2_2_3.xlsx') {
  ...
  const rules = [
    ...
    ['A4 Transport Stage', 'Consolidated freight — Transport Activity (t·km) × Fleet Intensity', 'kg CO2e', 'DIN EN ISO 14083 / GLEC Framework, adopted app-wide 2026-07-27; the class template\'s round-trip formula is kept on this workbook as a reference/audit column, not the reported figure'],
  ...
  analysisSheet.columns = [
    ...
    { header: 'A4 Transport — Round-Trip (kgCO2e, reference)', width: 30 },
    { header: 'A4 Transport — Consolidated (kgCO2e, reported)', width: 30 },
  ...
  const aHeaderRow = analysisSheet.addRow([
    ...
    'A1-A3 Production (kgCO2e)', 'A4 Transport — Round-Trip (kgCO2e, reference)', 'A4 Transport — Consolidated (kgCO2e, reported)',
  ...
  ASSEMBLY_CONFIGS.forEach((cfg) => {
    ...
    assemblyRows.forEach((r, i) => {
      ...
      const cells = {
        ...
        // Unit rows (Door/Window/Skylight main product): a4 is the
        // round-trip REFERENCE figure — must come from r.a4RoundTrip, not
        // r.a4 (r.a4 is the app's canonical A4, consolidated since
        // 2026-07-27, and would double-count as both columns otherwise).
        a4: isUnitRow ? num(r.a4RoundTrip, 1) : {
  ...
  assemblySheetMeta.forEach((metaItem, idx) => {
    ...
    analysisSheet.addRow({
      ...
      'A4 Transport — Round-Trip (kgCO2e, reference)': { formula: `${S}!Q${tot}` },
      'A4 Transport — Consolidated (kgCO2e, reported)': { formula: `${S}!R${tot}` },
      ...
      // D=A1-A3, F=A4 Consolidated (the app's reported A4 as of 2026-07-27
      // — E, round trip, stays on the sheet as reference only and is
      // deliberately excluded from this total), G=B4, H=C1-C4 EOL.
      'Total Embodied GWP (kgCO2e)': { formula: `D${rowNum}+F${rowNum}+G${rowNum}+H${rowNum}` },
  ...
  const totRow = analysisSheet.addRow({
    ...
    'A4 Transport — Round-Trip (kgCO2e, reference)': { formula: `SUM(E${startSummaryRowIndex}:E${endSummaryRowIndex})` },
    'A4 Transport — Consolidated (kgCO2e, reported)': { formula: `SUM(F${startSummaryRowIndex}:F${endSummaryRowIndex})` },
